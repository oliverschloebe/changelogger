...
Copyright 2010-2026 Oliver Schlöbe (email : wordpress@schloebe.de)
...
class Changelogger {
	...
	* PHP 5 Constructor
	...
	public function __construct() {
		...
		add_action('after_plugin_row', [&$this, 'display_info_row'], 50, 2);
		add_action('wp_ajax_clos_ajax_load_changelog', [&$this, 'clos_ajax_load_changelog']);
		add_filter('pre_update_option_active_plugins', [&$this, 'flush_changelog_cache']);
	}
	...
	* PHP 4 Compatible Constructor
	...
	public function enqueue_scripts_and_styles() {
		wp_enqueue_script(
			'clos-generalscripts',
			plugins_url( 'js/admin_scripts.js', __FILE__ ),
			['jquery', 'sack'],
			CLOSVERSION
		);
		wp_enqueue_style(
	...
	public function init() {
		...
		if( $pagenow == 'plugins.php' && !isset( $_GET['action'] ) ) {
			...
			add_action('admin_print_scripts', [&$this, 'js_admin_header']);
		}
	}
	...
	public function display_info_row( $file, $plugin_data ) {
		...
		if (false === $output) {
			$api = plugins_api('plugin_information', ['slug' => $r->slug, 'fields' => ['tested' => true, 'requires' => false, 'rating' => false, 'downloaded' => false, 'downloadlink' => false, 'last_updated' => false, 'homepage' => false, 'tags' => false, 'sections' => true] ]);
			if ( !is_wp_error( $api ) && current_user_can('update_plugins') ) {
				...
				if( isset($api->sections['changelog']) ) {
					...
					if( $section_exists ) {
						$search = ["<strong>Version ", "<h4>Version ", "Version ", "<strong>v", "<h4>v"];
						$replace = ["<strong>", "<h4>", "", "<strong>", "<h4>"];
						...
						$l_arrw = '&laquo; ';
						$r_arrw = ' <a href="#" onclick="clos_ajax_load_changelog( \'' . $r->slug . '\', \'1\' );return false;" title="' . esc_attr__('Previous version') . '" class="clos-arrw clos-arrw-r">&raquo;</a>';
	...
	public function clos_ajax_load_changelog() {
		check_ajax_referer( 'clos_load_changelog', 'security' );

		if( !current_user_can( 'update_plugins' ) ) {
			wp_die( '-1', 403 );
		}

		$sectionid = isset( $_POST['sectionid'] ) ? absint( $_POST['sectionid'] ) : 1;
		$pluginslug = isset( $_POST['pluginslug'] ) ? sanitize_key( $_POST['pluginslug'] ) : '';
		if( '' === $pluginslug ) {
			wp_die( '0', 400 );
		}
		...
		if ( $current_changelog ) {
			...
			die("// Cached output" . chr(012) . "jQuery('div#clos-message-" . esc_js( $pluginslug ) . "').fadeOut('fast', function() { jQuery(this).html(\"" . str_replace( chr(012), "", wp_slash( $versioninfo ) ) . "\").fadeIn('normal') });");
		}
		
		include_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
		$api = plugins_api('plugin_information', ['slug' => $pluginslug, 'fields' => ['tested' => false, 'requires' => false, 'rating' => false, 'downloaded' => false, 'downloadlink' => false, 'last_updated' => false, 'homepage' => false, 'tags' => false, 'sections' => true] ]);
		if( isset($api->sections['changelog']) ) {
			$changelog = $api->sections['changelog'];
			$section_exists = preg_match_all('/(<h4>|<p><strong>|<strong>|[\n|\r]{0,})(.*)([\n|\r]{0,}|<\/strong>|<\/strong><\/p>|<\/h4>)[\n|\r]{0,}<ul>(.*)<\/ul>[\w|\W]{0,}/isU', $changelog, $changelog_result);
			if( $section_exists ) {
				...
				if( trim($changelog_result[0][$sectionidl]) != '' || !empty($changelog_result[0][$sectionidl]) )
					$l_arrw = sprintf('<a href="#" onclick="clos_ajax_load_changelog( \'%1$s\', \'' . $sectionidl . '\' );return false;" class="clos-arrw clos-arrw-l" title="' . esc_attr__('Next version') . '">&laquo;</a> ', esc_js( $pluginslug ));
				else
					$l_arrw = '&laquo; ';
				
				$sectionidn = ($sectionid+1);
				if( trim($changelog_result[0][$sectionidn]) != '' || !empty($changelog_result[0][$sectionidn]) )
					$r_arrw = sprintf(' <a href="#" onclick="clos_ajax_load_changelog( \'%1$s\', \'' . $sectionidn . '\' );return false;" class="clos-arrw clos-arrw-r" title="' . esc_attr__('Previous version') . '">&raquo;</a>', esc_js( $pluginslug ));
				else
					$r_arrw = ' &raquo;';
				...
				die("jQuery('div#clos-message-" . esc_js( $pluginslug ) . "').fadeOut('fast', function() { jQuery(this).html(\"" . str_replace( chr(012), "", wp_slash( $versioninfo ) ) . "\").fadeIn('normal') });");
			}
		}
	}
 	...
 	* @author 		scripts@schloebe.de
 	*/
	public function _esc_attr__( string $str ) {
...
var clos_nonce = "<?php echo esc_js( wp_create_nonce( 'clos_load_changelog' ) ); ?>";
</script>
	<?php
	}
}
